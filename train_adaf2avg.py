...
"""AdaF2Avg: dynamic failure-aware FedAvg with evaluator-compatible exports.
...
ALGORITHM_ID = "adaf2avg"
ALGORITHM_NAME = "AdaF2Avg (Adaptive Failure-Focused FedAvg)"
FEATURES = (
...
def failure_focused_scores(
    sample_counts: np.ndarray, failure_rates: np.ndarray, current_lambda: float
) -> tuple[np.ndarray, float]:
    """Return AdaF2Avg scores; lambda=0 is exactly sample-size FedAvg."""
    if len(sample_counts) == 0 or len(sample_counts) != len(failure_rates):
        raise ValueError("Sample counts and failure rates must be non-empty and aligned")
    if np.any(sample_counts <= 0) or np.any(failure_rates < 0) or np.any(failure_rates > 1):
        raise ValueError("Invalid client sample counts or failure rates")
    global_failure_rate = float(np.average(failure_rates, weights=sample_counts))
    failure_lift = failure_rates / max(global_failure_rate, 1e-12)
    return sample_counts * (1.0 + current_lambda * failure_lift), global_failure_rate
...
def train_one(
    ...
    sample_counts = np.asarray([int(client["samples"]) for client in clients], dtype=float)
    failure_rates = np.asarray(
        [int(client["failures"]) / max(int(client["samples"]), 1) for client in clients],
        dtype=float,
    )
    ...
    for round_number in range(1, rounds + 1):
        ...
        for client in clients:
            ...
            losses.append(loss)
        aggregation_scores, global_failure_rate = failure_focused_scores(
            sample_counts, failure_rates, current_lambda
        )
        ...
        history.append({
            ...
            "global_train_failure_rate": global_failure_rate,
            "min_client_aggregation_multiplier": float(np.min(aggregation_scores / sample_counts)),
            "max_client_aggregation_multiplier": float(np.max(aggregation_scores / sample_counts)),
        })
    ...
    model = {
        "algorithm": ALGORITHM_ID, "algorithm_name": ALGORITHM_NAME,
...
def main() -> None:
    ...
    pd.DataFrame(all_predictions).to_csv(args.output_dir / "adaf2avg_predictions.csv", index=False)
    pd.DataFrame(all_history).to_csv(args.output_dir / "adaf2avg_training_history.csv", index=False)
    (args.output_dir / "adaf2avg_models.json").write_text(json.dumps(models, indent=2), encoding="utf-8")
    manifest = {
        "algorithm": ALGORITHM_ID, "algorithm_name": ALGORITHM_NAME, "seeds": seeds,
    ...
    (args.output_dir / "adaf2avg_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
