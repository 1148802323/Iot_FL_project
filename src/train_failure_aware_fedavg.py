...
def failure_aware_strategy(
    ...
    total_failures = sum(int(np.sum(c["y"])) for c in clients)  # type: ignore[arg-type]
    global_failure_rate = total_failures / max(total_rows, 1)
    history_rows = []
    ...
    for round_num in range(1, rounds + 1):
        ...
        for client in clients:
            ...
            client_failure_rate = float(client["failure_rate"])
            if global_failure_rate > 0:
                failure_lift = client_failure_rate / global_failure_rate
            else:
                failure_lift = 0.0
            aggregation_weight = client_rows * (1.0 + alpha * failure_lift)
        ...
        history_rows.append(
            {
                ...
                "alpha": alpha,
                "global_failure_rate": round(float(global_failure_rate), 6),
    ...
    model_payload = {
        "strategy": strategy,
        "method": "failure_aware_fedavg_weighted_logistic_regression",
        "features": FEATURES,
        "intercept": float(global_w[0]),
        "coefficients": {feature: float(value) for feature, value in zip(FEATURES, global_w[1:])},
        "threshold": threshold,
        "alpha": alpha,
        "global_failure_rate": float(global_failure_rate),
        "aggregation_formula": "client_samples * (1 + alpha * client_failure_rate / global_failure_rate)",
        "clients": [
            {"name": str(c["name"]), "rows": int(c["rows"]), "failure_rate": float(c["failure_rate"])}
            for c in clients
        ],
    }
...
def draw_baseline_comparison(comparison: pd.DataFrame, out_path: Path) -> None:
    ...
    img = Image.new("RGB", (width, height), "white")
    draw = ImageDraw.Draw(img)
    draw.text((75, 35), "Failure-Aware FedAvg vs Standard FedAvg", fill=COLORS["ink"], font=font(32, True))
    ...
    for strategy in STRATEGIES:
        ...
        for i, metric_name in enumerate(metric_cols):
            ...
            aware = float(row[f"failure_aware_{metric_name}"])
    ...
    draw.text((legend_x + 32, 82), "Failure-Aware FedAvg", fill=COLORS["ink"], font=font(17))
    draw.text((35, top + 22), "Score", fill=COLORS["ink"], font=font(18))
    draw.text((left + 10, bottom + 165), "Delta labels show Failure-Aware minus Standard FedAvg.", fill=COLORS["muted"], font=font(16))
    img.save(out_path)
...
def compare_with_fedavg_baseline(results_df: pd.DataFrame, baseline_path: Path) -> pd.DataFrame | None:
    ...
    for strategy in STRATEGIES:
        ...
        for metric_name in ["accuracy", "precision", "recall", "f1"]:
            ...
            row[f"failure_aware_{metric_name}"] = float(aware_row[metric_name])
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run Version 1 failure-aware FedAvg across AI4I factory splits."
    )
    parser.add_argument(
        "--data",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Strength of failure-aware aggregation. 0.0 is equivalent to sample-size FedAvg.",
    )
    ...
    models = {}
    for strategy in STRATEGIES:
        history, threshold_table, payload = failure_aware_strategy(
            strategy=strategy,
    ...
    history_df.to_csv(reports / "failure_aware_fedavg_history.csv", index=False)
    threshold_df.to_csv(reports / "failure_aware_fedavg_threshold_tuning.csv", index=False)
    results_df.to_csv(reports / "failure_aware_fedavg_results.csv", index=False)
    (processed / "failure_aware_fedavg_models.json").write_text(json.dumps(models, indent=2), encoding="utf-8")

    draw_convergence(history_df, figures / "failure_aware_fedavg_convergence.png")
    draw_final_metrics(results_df, figures / "failure_aware_fedavg_final_metrics.png")
    draw_loss_curve(history_df, figures / "failure_aware_fedavg_client_loss.png")
    comparison = compare_with_fedavg_baseline(results_df, reports / "fedavg_baseline_results.csv")
    if comparison is not None and not comparison.empty:
        comparison.to_csv(reports / "failure_aware_vs_fedavg_results.csv", index=False)
        draw_baseline_comparison(comparison, figures / "failure_aware_vs_fedavg_metrics.png")

    print("Failure-aware FedAvg complete.")
