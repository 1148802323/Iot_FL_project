...
def fedavg_strategy(
    ...
    for round_num in range(1, rounds + 1):
        ...
        for client in clients:
            ...
            client_weights.append((w, int(client["rows"])))
            client_losses.append(loss)
        global_w = sum(w * (n / total_rows) for w, n in client_weights)
    ...
    model_payload = {
        "strategy": strategy,
        "method": "fedavg_all_clients_weighted_logistic_regression",
        "features": FEATURES,
...
def draw_convergence(history: pd.DataFrame, out_path: Path) -> None:
    ...
    draw.text((75, 35), "FedAvg Baseline Convergence", fill=COLORS["ink"], font=font(34, True))
...
def draw_final_metrics(results: pd.DataFrame, out_path: Path) -> None:
    ...
    img = Image.new("RGB", (width, height), "white")
    draw = ImageDraw.Draw(img)
    draw.text((75, 35), "FedAvg Final Test Metrics", fill=COLORS["ink"], font=font(34, True))
...
def draw_loss_curve(history: pd.DataFrame, out_path: Path) -> None:
    ...
    img = Image.new("RGB", (width, height), "white")
    draw = ImageDraw.Draw(img)
    draw.text((75, 35), "FedAvg Mean Client Loss", fill=COLORS["ink"], font=font(34, True))
    left, right, top, bottom = 115, 1140, 130, 610
    draw.line((left, top, left, bottom, right, bottom), fill=COLORS["muted"], width=2)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run FedAvg baseline across AI4I factory splits."
    )
    parser.add_argument(
        "--data",
        type=Path,
    ...
    for strategy in STRATEGIES:
        history, threshold_table, payload = fedavg_strategy(
            strategy=strategy,
    ...
    history_df.to_csv(reports / "fedavg_training_history.csv", index=False)
    threshold_df.to_csv(reports / "fedavg_threshold_tuning.csv", index=False)
    results_df.to_csv(reports / "fedavg_baseline_results.csv", index=False)
    (processed / "fedavg_models.json").write_text(json.dumps(models, indent=2), encoding="utf-8")

    draw_convergence(history_df, figures / "fedavg_convergence.png")
    draw_final_metrics(results_df, figures / "fedavg_final_metrics.png")
    draw_loss_curve(history_df, figures / "fedavg_client_loss.png")
